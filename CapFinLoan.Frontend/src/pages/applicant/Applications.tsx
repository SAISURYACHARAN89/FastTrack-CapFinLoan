...
export function ApplicantApplications() {
  ...
  return (
    <DashboardLayout>
      {/* ── Header ─────────────────────────────────────────────── */}
      <header className="mb-8 lg:mb-12 flex justify-between items-end flex-wrap gap-4">
        <div>
        ...
        <Link
          to="/applications/new"
          className="inline-flex items-center gap-2 px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl bg-gradient-to-r from-primary-container to-secondary-container text-on-primary-fixed font-bold text-sm shadow-lg hover:shadow-primary/30 transition-all active:scale-95"
        >
      ...
      <section className="rounded-2xl bg-surface-container-low border border-outline-variant/5 overflow-hidden shadow-2xl">
        ...
        <div className="px-4 sm:px-8 py-5 flex flex-wrap justify-between items-center gap-3 border-b border-white/5">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[11px] uppercase tracking-widest text-slate-500 font-bold bg-surface-container-lowest/50">
                  ...
                  <th className="px-4 sm:px-8 py-4">Amount</th>
                  <th className="px-4 sm:px-8 py-4">Status</th>
                  <th className="hidden sm:table-cell px-4 sm:px-8 py-4">Submitted</th>
                  <th className="hidden lg:table-cell px-4 sm:px-8 py-4">Decision</th>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredApps.map(app => (
                  <tr key={app.id} className="hover:bg-white/5 transition-colors group">
                    <td className="px-4 sm:px-8 py-4 text-sm font-medium text-slate-400 font-mono">
                    ...
                    <td className="px-4 sm:px-8 py-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold ${statusStyles[app.status] ?? statusStyles['PENDING']}`}>
                        {app.status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="hidden sm:table-cell px-4 sm:px-8 py-4 text-sm text-slate-500 font-medium">
                      {new Date(app.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="hidden lg:table-cell px-4 sm:px-8 py-4 text-sm text-slate-500 max-w-xs">
                      {app.decidedAtUtc ? (
                        <div className="flex flex-col">
